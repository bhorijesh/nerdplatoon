...
class PropertyUpdater:
    ...
    def update_data(self, changed_data: dict) -> None:
        ...
        if 'img' in changed_data and isinstance(changed_data['img'], list):
            changed_data['img'] = json.dumps(changed_data['img'])
        if 'new_feature' in changed_data and isinstance(changed_data['new_feature'], dict):
            changed_data['new_feature'] = json.dumps(changed_data['new_feature'])
    ...
    def get_database_links(self) -> pd.DataFrame:
        query = "SELECT id, title, uuid, link, MLS_ID, Web_Id, price, price_unit, img, about, exterior, interior, bedroom, full_baths, partial_bath, Property_type, amenities, exterior_details, interior_details, Property_Details, new_feature FROM Bahamas WHERE deleted_at IS NULL"
        try:
